...
**開いた直後に部屋の全体が映る**ようにしてある（`sim/rooms.py` の `<statistic>`）。
それでも見失ったら、下の「引きの戻し方」を使う。

| やりたいこと | 操作 |
|---|---|
| **引く／寄る（ズーム）** | **スクロール**（トラックパッドは2本指上下）。または中ボタンドラッグ |
| 回す | 左ドラッグ |
| 平行移動 | 右ドラッグ（`Shift`+右ドラッグで上下） |
| **クリックした所を画面の中心にする** | **右ダブルクリック** |
| クリックした物を追いかける | `Ctrl`+右ダブルクリック |
| カメラを切り替える | `[` / `]` |
| 物を選ぶ | ダブルクリック |
| 左右のパネルを隠す／出す | `Tab` / `Shift`+`Tab` |
| **操作一覧をウィンドウ内に出す** | **`F1`** |

**引きの戻し方**（部屋を見失ったとき）:

1. スクロールを下方向に何回か回す
2. それでも駄目なら**右ダブルクリック**で床の真ん中を画面中心に置き直す
3. `]` を押すと機体を追う `chase` カメラに切り替わる。**歩いている様子を見るならこれがいちばん見やすい**

カメラは 2 つ用意してある:

| カメラ | 見え方 |
|---|---|
| free（既定） | 部屋の南側から 45° 見下ろす。全体が入る。自分で動かせる |
| `chase` | 機体の後ろ 4m・上 2.5m から追いかける。向きは world 基準なので歩容で画面が揺れない |

> ⚠️ **`Space`（Play/Pause）と `+`/`-`（速度）は効かない。** `launch_passive` の
> ビューアは物理を回さず、こちら（`sim/run_sim.py`）が回しているため。
> 速度を変えるのは `--speed`、止めたいときはウィンドウを閉じる
> （閉じると残りは全速で走り切って結果を出す）。
...
| **ビューアの初期カメラ** | `<statistic center=部屋の中心 extent=長辺×0.65>` + 真南から45°見下ろす | **書かないと部屋が映らない。** MuJoCo は `<statistic>` が無いとモデルの境界箱から extent を推定するが、床が無限平面で境界箱に寄与しないため**機体の手足の寸法だけ**で決まる（実測: 11m×6m の部屋で extent=1.23 になり床のテクスチャに寄り切った）。仰角も 45° より浅いと高さ2mの壁が視界を塞ぐ |
| **追従カメラ** | 機体の後ろ4m・上2.5m、`trackcom` | 歩く様子を見るのに要る。`trackcom` は位置だけ追従して向きは world 基準なので、歩容のロール・ピッチで画面が揺れない |
