...
機体を立てたまま放置しただけで、**`1801` から約16分で odom と points が両方止まった**
（3回とも 941〜1022 秒）。LiDAR の生点群は 9.97Hz で流れ続けるので**気づきにくい**。
バッテリ 77〜93% / CPU 51〜59% / 59〜66℃ と余裕のある状態で起きた。

⚠️ **判定に `/slam_info` の `"info": "not init"` を使ってはいけない。**
これは「1804 でナビ用地図を読み込んだか」を示すマーカーで、**建図中も "not init" のまま**
（[../findings/g1_dds_sensors.md](../findings/g1_dds_sensors.md) §8.3）。
**有効な判定は `/unitree/slam_mapping/odom` のレートだけ。**
...
⚠️⚠️ **この 80% は「校正した出発地点で、その場のスキャンを合わせた直後」の基準。
歩いたあとに測り直したときに当てはめてはいけない。**

| いつ | 20cm以内 | 50cm以内 |
|---|---|---|
| §7 直後（出発地点） | **97%**（中央値 5cm） | 99% |
| 歩いたあと・部屋の別の場所 | **44%** | 49% |

2026-09-15 の実測。**44% でも自己位置は正しかった**（利用者が RViz で目視確認し、
odometry と 6cm 以内で一致）。地図が 2026-09-07 取得で現状と合っていないため、
**地図に無い物・動いた物を見ているぶんが一致率から落ちる**。

📌 **歩いたあとに評価するなら「50cm以内」を見ること。** 80% を機械的に当てはめると、
正しく合っているのに §7 をやり直し続けることになる。
📌 **一致率の低さ ＝ 自己位置の誤り、ではない。** 最終的な判定は
**RViz で機体の位置と向きが実機と合っているか**の目視。
...
**巡回路をまず作る**（1回だけ。地図を作り直したらやり直す）。やり方は2つ。

**(1) RViz で引く（速い）** — 操作PC の RViz だけで完結し、機体は動かない

```bash
~/g1_nav2/tools/patrol_ctl.sh teach     # TEACH に入る
#  → RViz の **「Publish Point」**で回りたい順に地面をクリックする
#     ⚠️⚠️ **「2D Goal Pose」ではない。** あれは bt_navigator が直接拾うので、
#          クリックした瞬間に機体が本当にそこへ歩き出す
#  → 引いた形は **PatrolRoute**（マゼンタの線＋黄色の矢印）にその場で出る
~/g1_nav2/tools/patrol_ctl.sh undo      # 直前の1点を取り消す
...
# → 終わったら Nav2 を巡回路つきで上げ直す（g1up.sh --patrol <yaml>）
```

⚠️ **(1) でクリックするのは「地図の上の座標」。** room_a の地図は 9/07 取得で
現状と合っていないので、**地図では通れるように見えて実際には通れない点**を
作り込めてしまう。(2) が確実なのは、**機体が実際にそこに立てた**という事実が
座標の裏付けになるから。
📌 **実務的には併用。** (1) でざっと引いて1周流し、**着けなかった点だけ (2) で取り直す。**
📌 (1) の向きは「次の点へ向かう方位」が自動で入る。特定の向きで止まりたい点は
yaml の `yaw_deg` を書き換えること。
...
| **自己位置が合わない** | §7 をやり直す。判定は §7 のとおり — **校正直後は `20cm以内` 80%以上、歩いたあとは `50cm以内`**。最後は RViz の目視 |
