...
public:
    ...
    // cmd_timeout の計測を解除する(D1、2026-09-24)。
    //
    // ⚠️ **`cmd_timeout` は「Nav2 が死んだ」を捕まえるためのもので、
    // 「Nav2 が黙っている」を捕まえるためのものではない。** Goal が 1 件も
    // 走っていないときは Nav2 が指令を出さないのが正常なので、そのまま数えると
    // **Goal 到達の約 1.3〜3 秒後に必ず FAULT に落ちる**(2026-09-15 / 09-24 実機)。
    // 呼ぶと「まだ最初の指令を受けていない」状態に戻り、Tick は判定を見送る。
    // 次の指令が来た時点で計測が再開する。
    void SuspendCmdTimeout();
