...
#include <queue>
#include <cmath>
#include <stdexcept>  // vendoring修正(nav2_option): std::runtime_error用
                                             ...
                                             tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    // vendoring修正(nav2_option, 2026-09-09): "initialpose"のデフォルト値が空vectorのため、
    // paramsファイルのトップレベルキーがこのノード名("global_loc_node")と一致していないと
    // 何も上書きされず、下のindex参照(initialpose_[3]等)で範囲外アクセス・SIGSEGVしていた。
    // 原因のconfig側の不一致(loc_param_g1.yaml)は別途修正済みだが、それとは独立に、
    // 起動時にパラメータの形が不正ならクラッシュではなく明確なエラーで止まるようにする。
    if (initialpose_.size() != 6)
    {
        RCLCPP_FATAL(this->get_logger(),
                     "parameter 'initialpose' must have exactly 6 elements "
                     "(x, y, z, roll, pitch, yaw) but got %zu. "
                     "paramsファイルのトップレベルキーがノード名 '%s' と一致しているか確認すること。",
                     initialpose_.size(), this->get_name());
        throw std::runtime_error("invalid 'initialpose' parameter (see log above)");
    }
