...
実データはこれと決定的に異なるため、**現在の場所は room_a ではない**と判断できる。
目視でも、live の構造は 55m × 33m に散在しており room_a の部屋（約 24m × 32m）の
輪郭と一致しない。

### 6.3 したがって

- **room_a の地図で Phase 2a を検証するには、ロボットを物理的に room_a へ移す必要がある**
- あるいは現在地の地図を新規に作る（その場合 Mapping 側のパイプラインを使うが、
  既定で `CAMERA_REQUIRED=true` なのでカメラを繋いだままにするか設定を変える）
...
### 9.3 この試験の限界（geometry は意味を持たない）

**現在地は room_a ではない**（§6）ので、`map→odom` は恒等変換のまま置いた。
したがって:

- global costmap（room_a の地図）と local costmap（実機 LiDAR の今の部屋）は
  **別の場所を表している**
- 経路も到達判定も幾何的には意味が無い

**確認できたのは「配線が通ること」だけ**であり、自己位置推定や経路の妥当性は
別途 room_a で（あるいは現在地の地図を作って）検証する必要がある。
