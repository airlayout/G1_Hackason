...
| # | 決定事項 |
|---|---|
| D-24 | **MVP は「床に低い障害物が置かれていない管理された区域」に適用環境を限定する**。MID-360 のみで成立させる |
| D-25 | **D435i による死角補完は MVP 後の後続フェーズ（Phase 3）で追加する** |
...
| # | 項目 | 影響範囲 | 未確定時のリスク |
|---|---|---|---|
| U-07 | **`duration` 経過後に G1 が実際に停止するか、直前速度を保持し続けるか（正確な時間を含む）** | 安全設計の根幹 | `duration` 自体は D-27 で明示制御できることが確定済みだが、経過後の挙動（自動停止／保持）は SDK 内に明記が無く実機でしか確認できない |
| U-08 | **各速度での停止距離** | Collision Monitor / footprint / 速度上限 | 安全パラメータが根拠なしになる |
| U-09 | **MID-360 の実際の取付角度と死角範囲** | costmap 構成、MVP 適用環境の定義 | 死角の想定が外れる |
| U-10 | odometry の符号・単位の実測校正 | `g1_state_bridge`、Nav2 全体 | 方向が反転して暴走 |
| U-11 | その場旋回時の並進ドリフト量 | Nav2 の回転動作、localization | Goal 到達精度 |
| U-12 | 微小速度のデッドバンド閾値（歩容が成立する下限速度） | D-14 のパラメータ | Goal に到達しない |
...
| Phase 0 の観察結果 | 対応 |
|---|---|
| `duration` 満了後に自動的に停止する | 想定どおり。D-27 の方針のまま、SDK 側 watchdog を冗長層として運用 |
| `duration` 満了後も直前速度を保持し続ける（または不定な挙動） | D-27 の運用だけでは不十分。**SDK 側 watchdog（周期的なゼロ速度送信）を一次防衛線に格上げ**し、`duration` はあくまで補助とする。有線 LAN 必須・純正リモコン常時待機を受入要件に格上げする |
