...
> ## ⛔ この節の②の結論は**誤り**である。§10 を読むこと。
>
> ②で「足元の床が見えないこと（D-21 の前提）を実測で裏付けた」と書いたが、**逆だった。**
> **MID-360 は逆さ（roll≈180°）に取り付けられており、視野の広い側(52°)が下を向いている。**
> 立位で測り直した結果、**床は約1.0m 先から11.94m 先まで見えており、死角は半径約1m の円だけ**
> （床平面の inlier 623,965点、センサー高さ 1.213m）。
> 当時は脱力姿勢で床の反射がほとんど無く、視野の下限の直線を「床が見えない証拠」と
> 読み違えていた。**D-21 の前提そのものが誤っていた**（詳細と根拠: §10）。
>
> ①（垂直視野 -6.75°〜+50.50°）と③（機体自身が至近に写る）は正しい。
...
- nav2_stable の Phase 3（D435i による死角補完）は**深度**を使うので、いずれ
...
| 1804 が全 address で `errorCode 507` を返す | `Navigation/README.md` の過去の記録。**nav2_stable 側では 1804 を一度も送っていない** |
...
## 10. ⚠️⚠️ U-09 の確定と、それが覆した前提（2026-09-09、立位で実測）

**この節は D-21 / D-24 / D-25 の根拠を書き換える内容を含む。**

### 10.1 MID-360 は**逆さ**に取り付けられている

2つの IMU を同時に読んで確定した。

| | 加速度計の値（静止時 ＝ **上向き**）| 鉛直からの傾き |
|---|---|---|
| **本体IMU**（`LowState_.imu_state`）| `(-0.0560, +0.0182, +0.9983)` [m/s²正規化] | 3.37° |
| **Livox IMU**（`/utlidar/imu_livox_mid360`）| `(-0.0642, -0.0016, -0.9994)` [g] | 3.69° |

**z の符号が逆で、y も符号が反転している。** これは「Livox の座標系＝本体座標系を X 軸まわりに
180°回したもの」と一致する（本体の上向きを (x,-y,-z) に写すと Livox の測定値とほぼ一致）。
Mapping 班の較正値 `--livox-rpy-deg 178.35 -8.41 -0.72`（**ロール約178°**）と整合する。

⚠️ **静止した加速度計が返すのは「重力」ではなく重力の反作用＝上向き**。ここを取り違えると
符号を間違える（実際に間違えた。§10.4）。

### 10.2 点群も逆さのまま配信されている（＝床がよく見える）

`livox_frame` の点群の +z は**世界の下**を向いている。立位で取得した 1,603,680 点を
RANSAC で平面フィットして確定した。

| 領域 | 結果 |
|---|---|
| **cloud z>+0.3（＝世界の下側）** | **620,812 点(71%)が1枚の水平面**。法線 `(+0.066,+0.010,+0.998)`、水平から **3.81°**、**センサーからの距離 1.213 m**、広がり半径 10.1 m → **これは床**（立位 G1 の頭部高さと一致） |
| cloud z<-0.3（＝世界の上側） | 法線が水平から **85.92°** ＝ **垂直な壁**。水平な天井面は見えていない |

補強証拠: **至近(r<0.3m)の自己観測 374,745 点が z ≈ 0.000〜+0.145** に出る。
頭部 LiDAR の真下にあるはずの頭・首の構造が **+z 側**に見えている。

### 10.3 死角範囲の実測 ＝ **半径約1m**（D-21 の想定 10.6m は誤り）

床平面の inlier 623,965 点の水平距離分布:
...
方位別（30°刻み、12方位すべて）: **0.95〜1.12 m** でほぼ均一。

**理論値との照合が決定的**:

| 前提 | 床が見え始める理論距離 | 実測 |
|---|---|---|
| **視野の広い側 52° が下向き（＝逆さ取付）** | **0.95 m** | **0.91〜1.12 m** ✅ 一致 |
| 視野の狭い側 7° が下向き（＝D-21 の前提） | 9.88 m | ❌ 矛盾 |

**したがって D-21 の「足元〜10m の床面と低障害物が原理的に見えない」は誤り。**
床は約 1.0m 先から 11.9m 先まで見えており、**死角は半径約1m の円だけ**。
→ **D-24（MVP の適用環境限定）の根拠が弱まり、D-25/Phase 3（D435i による死角補完）の
優先度も下がる**（Planning.md 側に反映済み）。

### 10.4 自分のバグの記録: 加速度計の符号を取り違えた

`tools/g1_slam_odom_tf.py` の `--auto-level`（§9.2）で、IMU の `linear_acceleration` を
「重力」だと思って `(0,0,-1)` に合わせていた。**加速度計が返すのは上向き**なので、これは
**「上」を -z に向ける＝z軸が下向きのフレーム**を作っていた（ROS の base_link/odom/map は
z 上向きが規約）。

さらに悪いことに、検証用の `tools/check_gravity_tf.py` も `(0,0,-1)` を正解として
判定していたため、**バグの側を「残差 0.06°」と確認していた**（循環していた）。

**修正後**（実機で再検証）:

```
map<-livox_frame: 上向きを変換 → (+0.0007, -0.0003, +1.0000)  +Z軸からのずれ 0.04°
自動校正した: 生センサーの傾き 3.67° / map系で上向きが +z から 0.000°
```

**影響範囲**:
- §9 の Nav2 配線試験（A-10）は、**local costmap が上下逆のフレームで動いていた**。
  ノードは起動し `/cmd_vel` も出たので「配線が通る」ことの確認としては有効だが、
  **costmap の中身（`min_obstacle_height`/`max_obstacle_height` が選ぶ高さ帯）は誤っていた**。
  修正後にやり直す必要がある
- §6 の地図照合も、この符号の取り違えで「床」を誤って同定していた可能性が高い。
  §6.3 では「視野が上向きに偏るため天井偏重で照合できない」と説明したが、
  **実際は視野は下向きで床がよく見えていた**。§6 の失敗の原因は、より単純に
  **上下反転したフレームで高さ帯を切っていたこと**である可能性が高い（要再試験）

### 10.5 U-09 のまとめ（Planning.md / チェックシート項目6 に記録済み）

| 項目 | 実測値 |
|---|---|
| 取付向き | **逆さ（roll≈180°）**。視野の広い側(52°)が下 |
| センサー高さ（立位） | **1.213 m** |
| センサーの傾き | **3.81°**（床平面法線から。傾きの方位は cloud X 軸から +8.7°） |
| **死角半径** | **0.91〜1.12 m**（12方位ほぼ均一） |
| 床の可視範囲 | 約 1.0 m 〜 **11.94 m** |
| 計測時の姿勢 | 立位（膝 34.9°/36.1°、脚トルク計 45.3 Nm、本体IMU pitch +2.55°） |

### 10.6 フレームの修正完了と実機検証（2026-09-09）

§10.4 の符号バグに加え、**並進も未設定（`(0,0,0)`）だった**ことが分かった。
つまり **base_link がセンサー位置（床から1.2m）にあった**ため、costmap の高さ帯が
1.2m ずれていた。U-09 でセンサー高さが確定したので両方を直した。

| | 修正前 | 修正後 |
|---|---|---|
| 回転 | 上向きを (0,0,-1) に合わせていた（z下向きフレーム） | 上向きを **(0,0,+1)** に合わせる（ROS 規約の z 上向き） |
| 並進 | `(0,0,0)`（base_link＝センサー位置） | **`(0,0,1.213)`**（base_link＝床面。U-09 の実測値） |

**実機での最終検証**（点群を TF で base_link へ変換して床の高さを見る）:

```
base_link<-livox_frame: 並進 (+0.000,+0.000,+1.213)
base_link 系での z のピーク: +0.18 / +0.13 / -0.07 / +0.08 m   ← 床が z≈0 ✅
```

**床が base_link 系で z≈0 に来た。** これで costmap の
`min_obstacle_height: 0.05` / `max_obstacle_height: 1.8` が**床基準の障害物の高さ帯**を
正しく選ぶ。検証スクリプト: `tools/verify_frame.py` 相当（本節のコードを参照）。

⚠️ **残っている作業**: §9 の Nav2 配線試験（A-10）は誤ったフレームで走らせたので、
**修正後のフレームでやり直す**必要がある。ノードが起動し `/cmd_vel` が出ることは
確認済みだが、local costmap の中身は当時誤っていた。
