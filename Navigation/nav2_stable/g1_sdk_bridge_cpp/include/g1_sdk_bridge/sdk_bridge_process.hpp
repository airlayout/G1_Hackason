...
struct SdkBridgeConfig {
    ...
    // ⚠️ **モック専用。既定 0 は「無効」で、実機の挙動は一切変わらない。**
    //
    // 二足は指令が小さすぎると歩容が成立せず、**まったく動かない**
    // (U-12 実測: vx=0.1 は不成立 / 0.2 は歩くが方向が定まらない / 0.3 で実用的。
    //  角速度は 0.02 rad/s で不動、0.3 で回る)。
    // この「最小作動閾値」を入れないと、モックは指令どおりに完璧に動いてしまい、
    // **2026-09-15 に実機で踏んだ旋回デッドロックを再現できない**:
    //   RPP は「実測角速度 + 加速度上限 × dt」で指令を作るので、機体が動かないと
    //   実測が 0 のままになり、指令が 1 周期ぶんの増分から永久に増えない。
    // ここを模すことで、実機を出さずに回帰テストできる。
    double gait_min_vx = 0.0;     // |vx| がこれ未満なら並進しない [m/s]
    double gait_min_omega = 0.0;  // |omega| がこれ未満なら旋回しない [rad/s]
