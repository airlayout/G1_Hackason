...
| 管理単位 | **Nav2 方式の独立トラック**。`Navigation/` 直下の実装とは別系統として `nav2_stable/` 配下で管理する |
...
- 残作業: **controller(経路追従)側の検証はまだ**。上記のplanner検証は**Humble**のNav2(1.1.20)で行っており、nav2_stable本体の`g1_ws`は**Jazzy**向け(D-01)なので`nav2_params.yaml`そのままでの疎通は別途確認が必要。高さフィルタの既定値はPhase 0のU-09確定後に再調整の可能性あり
...
⚠️ **「現状に近いか」は測っていない**（同じスキャンを両方に当てる必要があり、機体が要る）。
📌 **2026-09-20、その比較はやらないと決めた（利用者の判断）。** 構造の比較で十分とし、
新しい地図でそのまま進める。現地の時間は①旋回の確認・巡回路の作成・③16分停止の
切り分けに使う。§7 の一致率が出発地点で 80% を切ったら、そこで初めて旧地図と比べる。
...
**本プロジェクトの調査成果物（`nav2_stable/findings/`）**
