...
| `check_lidar3d.py` | Isaac Sim 上での点群検証 | 通過 |

`check_lidar3d.py` の結果（32 層 × 360、前傾 20 度、Warehouse）:

| 項目 | 実測 |
|---|---|
| 有効な点 | 11446 / 11520（99.4%） |
| センサ高さ | 1.10 m（設計どおり） |
| **地上 0.3 m 未満の点** | **8872 点**（最も近いものは 0.39 m 先） |
| センサより 0.5 m 高い点 | 1546 点 |
| センサ座標変換の誤差 | 最大 0.000013 m |

**足元が見えることを実測で確認できた。** 2D LiDAR は地上 1.1 m の断面しか
見ないため 0.3 m 未満の点は 1 つも取れなかったが、3D では 8872 点取れており、
最も近い点は 0.39 m 先にある。これが「パレットに引っかかる」課題への回答。
...
| **32** | 2.83 | 3.07 | **3.42** | 4.13 |
| **64** | 3.04 | 4.18 | 4.09 | 5.16 |

**ビーム数を 512 倍（90 → 46080）にしても 2.69 ms → 5.16 ms（1.9 倍）。**
レイキャストは完全に固定費に支配されており、**3D 化のコストは実質ゼロ**。
最大条件の 46080 ビームでも 50Hz（20 ms）に対して 4 倍の余裕がある。
...
## はまり点：`data.pos_w` は offset を含まない（距離が 28% ずれる）

**`RayCaster.data.pos_w` をレイの発射位置として使ってはいけない。**

IsaacLab の `ray_caster.py` は `cfg.offset.pos` を**レイの始点にしか適用せず**、
`data.pos_w` には含めない（217 行目で `ray_starts += offset_pos`、233 行目の
`pos_w` は別経路）。
...
`data.pos_w` を原点に距離を計算すると、足元の点で **0.32 m（28%）過小評価**する。
床への当たり点で検算した（正しい 1.157 m に対し 0.838 m）。

`position_w` プロパティで offset を足して返すよう修正した。3D 版は姿勢で
offset を回してから足す（`ray_alignment="base"` のため）。2D 版はレイが常に
水平なので z に足すだけでよい。

**この不具合は `SimEnvTest` の 2D LiDAR にもある。** 地上 1.1 m を見ている
つもりで 0.745 m を原点に距離を出していたことになる。

### 見つけ方
`check_lidar3d.py` が「センサ高さ 0.75 m」と報告したのが手がかり。
`pelvis` / `torso_link` / `head_link` がすべて同じ 0.745 m を返すのも
異常なサイン（別の body なのに同じ高さになるはずがない）。
`src/probe_torso_height.py` と `src/probe_ray_origin.py` が切り分け用。
...
2. **実走行でパレットへの固着が消えるか確認する**（ステップ 7）。
   ...
   確認済みなので、あとは巡回ロジックがそれを使えるかどうか。
   現状の `patrol.py` は 2D スキャン前提なので、3D を見るように直す必要がある。

3. **`SimEnvTest` 側に 2 つの修正を適用するか決める。**
   どちらも動作実績のある環境に手を入れることになるので、判断を残してある。
   - `expand_mesh_paths`（146 倍高速化）: 実時間比 0.23x が大きく改善する見込み
   - `position_w` の offset 修正: 2D 側の影響は**軽微**（下記）

   **2D への影響は小さい。** `ray_alignment="yaw"` でレイが水平なので、
   誤差は「原点を 0.347 m 下にずらした斜辺」ぶんしか出ない。しかも符号は
   **過大**評価（安全側）:
   ...
   **既存の地図（`maps/warehouse.pgm`）への影響も無い。** `build_map.py` は
   `position_w` ではなく odom の `(x, y)` をレイ原点に使い、水平投影で格子を
   焼くため、鉛直方向の offset が入る経路が無い。地図の作り直しは不要。

   一方 **3D では 28% と大きい**。前傾で下を向いたレイが足元に当たるため、
   斜辺の効果が支配的になる（だから 3D 側は必ず直す必要があった）。
