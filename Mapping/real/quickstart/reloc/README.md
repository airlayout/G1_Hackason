...
```
1. 較正  信頼できる姿勢で **帯に切って** --residuals → 基準値 r0（この部屋で 0.070 m）
2. 探索  **全点で** --center/--roi（ROI ±3 m / 0.5 m・30° なら約 1.1 s）
3. 採否  返った姿勢を **帯に切って** --residuals で測り直し、
         r > N x r0 なら「自信なし」。一致率が低ければ「地図の外」。N は段 8 で決める
```
