...
if [ -z "$Q" ]; then
    ...
    # ⚠️⚠️ **品質が正常域でも、違う場所に居ることがある（2026-09-13 に実機で踏んだ）。**
    # MOLA が 2.26 m・16.4° 離れた偽の極大に収束していたのに、品質は 0.839 と正常域で、
    # この検査は「歩かせてよい」と答えていた。しかも**正しい姿勢の方が品質は低かった**
    # （誤 0.855 対 正 0.818）。**品質値は場所の正しさを何も保証しない。**
    warn "  ⚠️ **品質は「場所が正しい」ことの証拠にならない。**歩かせる前に重畳で裏を取ること"
    echo "     → bash quickstart/record_still.sh 60      # 重畳 [%] が出る（70% 以上なら合っている）"
    echo "       30% 台なら偽の極大に居る。全域探索で入れ直す（README-nav2 §4「測位が違う場所に…」）"
else
