...
## なぜ `inflation_radius` が効くのか — 「壁からこれだけ離れたい」の希望距離

`inflation_layer` は **`inflation_radius` を超えた所のコストを 0 にする**。
つまりこの値は「**壁からこれだけ離れて歩きたい**」という希望距離そのもので、
それを超えた分はプランナにとってご褒美が無い。

既定の 0.35 m では、余裕 0.36 m の廊下と 1.5 m の広間が
**まったく同じ（どちらもコスト 0）に見える**。だから幾何的に短い方＝狭い方を選ぶ。

掃引すると **選ばれる経路の最狭部 ≒ min(inflation_radius, 到達可能な最大ボトルネック)**
になる（実測、区間 wp7→wp2）:

| inflation_radius | 0.35 | 0.40 | 0.45 | 0.50 | 0.55 | 0.60 | 0.65 | 0.80 | 1.20 |
|---|---|---|---|---|---|---|---|---|---|
| 経路の最狭部 | 0.36 | 0.41 | 0.50 | 0.51 | 0.57 | 0.60 | 0.60 | 0.60 | 0.60 |

0.60 で飽和するのは、この区間の最大ボトルネック幅が 0.60 m だからである。
**だから「狙う幅を少し超える値」で足りる。それ以上は経路が長くなるだけ。**

## `cost_scaling_factor` はほぼ効かない（変えない）

`inflation_radius` を 0.70 に固定して掃引すると、**1.0 / 2.0 / 3.0 / 5.0 は
すべて同じ結果**（経路 18.11 m・最狭部 0.60 m）。10.0 で初めて悪化する
（0.57 m）——減衰が速すぎて「広い」と「そこそこ広い」の差が消えるため。
**現状の 5.0 は妥当なので触らない。変更は `inflation_radius` の 1 個だけにする。**
...
# ⚠️ 提案は **inflation_radius の 1 個だけ**。cost_scaling_factor は 5.0 のまま。
# 掃引すると sc は 1.0〜5.0 で結果が同じで、10.0 で初めて悪化する（0.60 → 0.57 m）。
CURRENT = (0.25, 0.35, 5.0)
PROPOSED = (0.25, 0.65, 5.0)
...
def main() -> None:
    ...
    print(f"{'robot_r':>8}{'inflation_r':>12}{'scaling':>9} | "
          f"{'経路長':>8}{'最狭部':>8}{'狭い所':>8}")
    # ⚠️ cost_scaling_factor は 5.0 で固定して inflation_radius だけ動かす。
    # 最狭部 ≒ min(inflation_radius, 最大ボトルネック) になるのが見える。
    for cfg in ((0.25, 0.35, 5.0), (0.25, 0.40, 5.0), (0.25, 0.45, 5.0),
                (0.25, 0.50, 5.0), (0.25, 0.55, 5.0), (0.25, 0.60, 5.0),
                (0.25, 0.65, 5.0), (0.25, 0.70, 5.0), (0.25, 0.80, 5.0),
                (0.25, 1.00, 5.0), (0.25, 0.70, 10.0)):
