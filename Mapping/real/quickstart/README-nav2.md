...
> **✅ 2026-09-13: 2 と 1 に手を入れた（`Navigation/nav2/g1_nav2.yaml`）。挙動は実機で未検証。**
>
> - **2 の原因**（並進しか数えない）は `progress_checker` を
>   **`nav2_controller::PoseProgressChecker`** に替えて潰した。
>   `required_movement_angle: 0.20`（11.5°）だけ回れば「進んでいる」と数える。
>   **Humble に既製で入っている**ので自作は要らない。半径を下げる対症療法もやめた
> - **1 の原因**（向きが合わない）は `yaw_goal_tolerance` を **0.35 → 3.15**（＝ 向きを問わない）。
>   まず位置だけで通し、通ったら **0.79 rad（45°）** に締める
>
> ⚠️ **2 つ同時に入っている。**`recoveries` が 0 にならなかったら**分けて測る**こと。
> ⚠️ **RViz2 の「2D Goal Pose」はドラッグの向きが yaw になる。**
> クリック運用では**進みたい向きへドラッグする**（ぽちっと押すだけにしない）。これは運用で効く。
...
- `--max-stray M` … 逸脱ガード。開始点から「直線 + M」を超えたら即キャンセル。
  **実機では必ず付ける**（復帰動作で想定外の方へ行ったとき timeout を待たずに止まる）。
  ⚠️ **これは自分が投げたゴールしか守れない。RViz2 からクリックするなら
  `stray_guard.py` を常駐させる**（下記）
- 部屋の座標（`--waypoints`）を使う場合、向きは**機体からゴールへの方位**が入る

### RViz2 からクリックするときの見張り（2026-09-13 に強化した）

```bash
docker exec -d -u ubuntu ... rviz bash -c 'source /opt/ros/humble/setup.bash && \
  python3 .../stray_guard.py --no-sim-time --max-abs 15.0 --max-seconds 120'
```

**投げ方に関係なく効く**ので、RViz2 のクリック（`/goal_pose`）も守れる。見るのは 3 つ:

| 何を | しきい | なぜ |
|---|---|---|
| 開始点からの距離 | `直線 + 0.5`、上限 `--max-abs` | もとからある首輪 |
| **見かけの速さ**（門番 G0） | **0.72 m/s**（上限 0.361 × 2.0）/ 窓 1.0 s | **ゴールの遠近に依らない。**推定が機体に出せない速さで動いていたら測位の破綻 |
| **`/tf` の途絶** | **3 s** | 止まると G0 も首輪も黙る。09-12 の崩壊は最後に `/tf` ごと止まった |

⚠️ **`--max-abs` の既定 3.0 は頭打ち。**4.35 m のクリックでも 3.0 m で鳴る（09-12 に踏んだ）。
クリック運用では**明示的に 15.0 を渡す**。広げるぶんは門番 G0 が受け持つ。
⚠️ **`--no-sim-time` を忘れない。**実機には `/clock` が来ないので時間判定と `/tf` 途絶が止まる。
⚠️ `preflight.sh` が**引数まで**見る（`--max-abs` / `--no-g0` / `--no-sim-time`）。

しきい値の根拠と検証（歩行 3 本・静止 7 本で 10/10）は
`docs/plan/2026-09-13-rviz-click-walk-live.md` §2.4.2。再現は `test_stray_guard_g0.py`。
