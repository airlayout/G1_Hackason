...
| `run_*_visual.py` | 検出＋検出枠を描いた画像の保存・表示 | ＋ultralytics/torch | 開発・検証（目視で確かめる）、リアルタイム確認 |
...
動画・画像・検出結果は、リポジトリのルートの`_local/`配下に置く。`_local/`は
上流の`.gitignore`で除外されており、**このPCにだけ置く（共有しない）データの区画**。

```
_local/perception/
├── videos/          【元データ】スマホ等で撮ったままの動画。★消さない
├── videos_640/      【生成物】ffmpegで640×480に変換した動画。videos/から作り直せる
│
├── probe/           probe_zmq_camera.py の出力（--out-dir の既定値）
│   ├── sim/         【生成物】シムの画像。撮り直せる
│   └── real/        【元データ】実機の画像。★バックアップ必須（実機はいつでも使えない）
│
└── visual/          run_*_visual.py の出力（--visual-dir の既定値）
    ├── sim/
    │   ├── dist_3m/          ← 1回の実行 = 1フォルダ（--run-name で命名）
    │   │   ├── images/
    │   │   ├── run.csv
    │   │   └── run.jsonl
    │   └── 2026-09-17_1830/  ← --run-name を省略した実行（日時が名前になる）
    └── real/
```

**分け方の基準は「どのスクリプトの出力か」の1つだけ**。評価用と確認用でフォルダを
分けたりはしない（境界が曖昧で、判断が要る分け方は続かないため）。実行ごとに
フォルダが分かれるので、`--run-name`に条件名（`dist_3m`など）を付ければ
そのまま評価の記録になる。

⚠️ **バックアップが必要なのは`videos/`と`probe/real/`の2つだけ。** どちらも
作り直せない元データ。それ以外はコマンドを再実行すれば再生成できる。

`run_sim.py` / `run_real.py`（軽い方）の検出結果だけは、上流の設計のまま
`Perception/<sim|real>/outputs/`に出力される（こちらも`.gitignore`で除外済み）。
