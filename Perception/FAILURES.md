...
## 2026-09-09: G1のPC2に、以前構築したはずのlerobot環境が存在しなかった

**何が起きたか**

ラボPCから実機G1のPC2（192.168.123.164）へ手動でSSHログインし、
`run_g1_server.py --camera`の起動状況を確認しようとしたところ、以下が判明した:

- `which python3.12` → 該当なし
- `python3 --version` → Python 3.8.10（システムのデフォルト）
- `conda`コマンドが見つからない（command not found）

しかし`G1_HuggingFace/note`の2026-08-21のエントリには、このPC2上にMiniforge・
conda環境"lerobot"（Python 3.12）・CycloneDDS・unitree_sdk2py・lerobotを構築し、
`run_g1_server.py`の起動確認まで済ませたと記録されている。今回確認した状態は、
その構築に着手する**前**の状態（「G1はUbuntu 20.04.6 LTS、Python 3.8.10、conda無し、
unitree_sdk2py/lerobot未導入」）と一致していた。

**原因**

未確認。以下のいずれかが考えられるが、チームへの確認が必要:

a) 2026-08-21にセットアップしたG1とは別個体に接続している
b) 同じ個体だが、PC2が何らかの理由でリセット・再インストールされた
c) 環境は別のユーザーアカウント配下に存在し、単に今回ログインしたアカウントから
   見えていないだけ

**反省**

作業ログに「構築済み」と記録されている内容を、実機側で再確認せずに前提としていた。
G1のような共有ハードウェアは、他メンバーの作業やトラブル対応で個体・OS状態が
記録と食い違っている可能性があることを考慮すべきだった。

**再発防止策**

この状態を確認した時点で、`run_g1_server.py`の起動やlerobot環境の再構築には
進まず作業を停止し、チームへの確認を優先した（このエントリもその記録。詳細な
時系列は`G1_HuggingFace/note`の2026-09-09エントリを参照）。次にこのG1個体・PC2で
作業する人は、作業開始前に必ず`python3 --version` / `which conda` /
`ls ~/*/lerobot`等で環境の実在を確認すること。もし本当に環境が失われていた場合は、
`G1_HuggingFace/note`の2026-08-21エントリ「実機G1側の環境構築」セクションの手順を
再実行すれば復元できるはずである。
