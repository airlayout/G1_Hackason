#!/usr/bin/env python3
"""Perception / real 検証用エントリスクリプト（検出結果を画像として保存・表示する版）。

run_real.py との違いは「検出枠を描いた画像を保存・表示できるかどうか」。
数値(console/JSONL/CSV)だけでは検出が正しいか人間が判断できないため、
目視で確認するために画像を残す、またはその場でウィンドウに表示する。

  - run_real.py        … 数値のみ。軽いので実機のCPUでの本番運用向け
  - run_real_visual.py … 数値＋画像の保存・表示。開発・検証向け（このファイル）
...
sim/run_sim_visual.py と処理は同一(参照するconfigが違うだけ)なので、**片方を直したらもう片方にも反映すること**。

使い方(実機G1側で run_g1_server.py --camera を起動しておくこと):
  cd Perception/real
  ../../G1_HuggingFace/venv/bin/python run_real_visual.py --max-frames 30
  ../../G1_HuggingFace/venv/bin/python run_real_visual.py --show   # qで終了
"""
import argparse
import sys
import time
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Perception / real: G1 YOLO検出パイプライン(検出結果を画像として保存・表示する版)",
