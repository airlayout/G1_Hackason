#!/usr/bin/env python3
"""Perception / real 検証用エントリスクリプト（検出結果を画像として保存する版）。

run_real.py との違いは「検出枠を描いた画像を保存するかどうか」だけ。
数値(console/JSONL/CSV)だけでは検出が正しいか人間が判断できないため、
目視で確認するために画像を残す。

  - run_real.py         … 数値のみ。軽いので実機のCPUでの本番運用向け
  - run_real_visual.py  … 数値＋画像。開発・検証向け（このファイル）
  - probe_zmq_camera.py … 認識せず、画像取得だけを確認する

sim/run_sim_visual.py と処理は同一で、参照する config が違うだけ
（接続先が実機G1のIPになる）。**片方を直したらもう片方にも反映すること。**
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Perception / real: G1 YOLO検出パイプライン(検出結果を画像として保存する版)",
