...
## bench_onboard.py（G1本体での推論性能の計測）

**最終的にPerceptionはG1本体(Jetson)で動かす想定**なので、そこで何msかかるかを
実機で測る必要がある。操作PCやラボPCの数値は参考にならない。

このスクリプトは`common/`に依存しない**単体で動くファイル**なので、G1本体へ
コピーするだけで実行できる。

### 準備（操作PC側）

```bash
# スクリプトと、YOLOの重みをG1へ渡す
scp Perception/real/bench_onboard.py unitree@192.168.123.164:~/
scp Perception/yolo26n.pt            unitree@192.168.123.164:~/   # 実機が外に出られない場合
```
...
```bash
source ~/miniforge3/bin/activate lerobot
pip install ultralytics          # 未インストールの場合(インターネット共有が必要)
python bench_onboard.py --weights ~/yolo26n.pt --out ~/bench_result.json
```

数分で終わる。`bench_result.json`を操作PCへ持ち帰って記録に残すこと。

### 何が分かるか

| 項目 | なぜ測るか |
|---|---|
| Jetsonの型番・JetPackの版 | これが分からないと他の数値を解釈できない |
| **`torch.cuda.is_available()`** | **GPUが使えるか。pipの標準wheelはaarch64でもCPU専用**なので、おそらくFalseになる |
| 入力サイズ別の推論時間 | 640で間に合うか、320まで落とす必要があるか |
| **スレッド数別の推論時間** | **Perceptionは他タスクとCPUを分け合う**。2スレッドに絞ったら何msかを知る |
| メモリ使用量(RSS) | Jetsonはメモリを全タスクで共有する。参考値: x86のCPU実行で約700MB |
| 連続動作時の速度低下 | 発熱による性能低下の有無。警備は長時間動かすため重要 |

### 資源を共有する前提での注意

**CPU・GPU・メモリはPerceptionの占有ではない。** 歩行制御・Mapping・Navigationと
同居する。以下を意識して設計すること。

- **歩行制御を妨げないことが最優先。** 転倒に直結する
- Perceptionが使ってよい上限（CPUスレッド数・メモリ）をチームで決めておく
- 実装側で守る手段:
  - `torch.set_num_threads(N)` でスレッド数を絞る（このスクリプトで影響を測れる）
  - 毎フレーム推論しない（間引く）
  - 動きがあったときだけ推論する（警備は何も起きない時間が大半）
  - `nice`/`taskset` で優先度や使用コアを制限する
- モデルを常駐させるだけで数百MBのメモリを使う。**モデルを大きくする判断は、
  速度だけでなくメモリの観点でも見ること**
